...
use crate::params::OasisDeEssParams;
use crate::presets;

#[derive(Lens, Clone)]
struct Data {
    params: Arc<OasisDeEssParams>,
    preset_name: String,
                    ...
                    let updates: Vec<(ParamPtr, f32)> = {
                        ...
                        preset.values.iter().filter_map(|&(param_id, norm_val)| {
                            let ptr = match param_id {
                                "frequency" => Some(params.frequency.as_ptr()),
                                "bandwidth" => Some(params.bandwidth.as_ptr()),
                                "threshold" => Some(params.threshold_db.as_ptr()),
                                "range" => Some(params.range_db.as_ptr()),
                                "mode" => Some(params.mode.as_ptr()),
                                "listen" => Some(params.listen.as_ptr()),
...
pub fn create_editor(params: Arc<OasisDeEssParams>) -> Option<Box<dyn Editor>> {
    ...
    create_vizia_editor(editor_state, ViziaTheming::Custom, move |cx, _| {
        ...
        VStack::new(cx, |cx| {
            ...
            HStack::new(cx, |cx| {
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "DETECTION").class("section-title");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Frequency", Data::params, |p| &p.frequency, false);
                            knob_control(cx, "Bandwidth", Data::params, |p| &p.bandwidth, false);
                        })
                        .class("knob-row");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Threshold", Data::params, |p| &p.threshold_db, false);
                            knob_control(cx, "Range", Data::params, |p| &p.range_db, false);
                        })
                        .class("knob-row");
                    })
                    .class("section");
                ...
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        ...
                        toggle_control(cx, "Listen", Data::params, |p| &p.listen);
                    })
                    .class("section");
