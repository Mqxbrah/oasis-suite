...
use crate::dsp::DeEssProcessor;
use crate::params::OasisDeEssParams;
use crate::ui;

pub struct OasisDeEss {
    params: Arc<OasisDeEssParams>,
    processor: DeEssProcessor,
    sample_rate_ctx: SampleRateContext,
}

impl Default for OasisDeEss {
    fn default() -> Self {
        Self {
            params: Arc::new(OasisDeEssParams::default()),
            processor: DeEssProcessor::new(),
            sample_rate_ctx: SampleRateContext::new(DEFAULT_SAMPLE_RATE),
        }
    }
}

impl Plugin for OasisDeEss {
    const NAME: &'static str = "Oasis DeEss";
    ...
    fn process(
        ...
        for mut channel_samples in buffer.iter_samples() {
            ...
            let bandwidth = self.params.bandwidth.smoothed.next();
            let threshold_db = self.params.threshold_db.smoothed.next();
            let range_db = self.params.range_db.smoothed.next();
...
impl ClapPlugin for OasisDeEss {
    const CLAP_ID: &'static str = "com.oasis-suite.oasis-deess";
    const CLAP_DESCRIPTION: Option<&'static str> = Some("Surgical sibilance control");
...
impl Vst3Plugin for OasisDeEss {
    const VST3_CLASS_ID: [u8; 16] = *b"OasisDeEssv1.0\0\0";
