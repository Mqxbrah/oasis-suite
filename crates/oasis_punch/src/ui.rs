                    ...
                    let updates: Vec<(ParamPtr, f32)> = {
                        ...
                        preset.values.iter().filter_map(|&(param_id, norm_val)| {
                            let ptr = match param_id {
                                "attack" => Some(params.attack.as_ptr()),
                                "sustain" => Some(params.sustain.as_ptr()),
                                "speed" => Some(params.speed.as_ptr()),
...
pub fn create_editor(params: Arc<OasisPunchParams>) -> Option<Box<dyn Editor>> {
    ...
    create_vizia_editor(editor_state, ViziaTheming::Custom, move |cx, _| {
        ...
        VStack::new(cx, |cx| {
            ...
            HStack::new(cx, |cx| {
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "TRANSIENTS").class("section-title");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Attack", Data::params, |p| &p.attack, true);
                            knob_control(cx, "Sustain", Data::params, |p| &p.sustain, true);
                        })
                        .class("knob-row");
                        toggle_control(cx, "Speed", Data::params, |p| &p.speed);
                    })
                    .class("section");
                })
                .class("column");
