                    ...
                    let updates: Vec<(ParamPtr, f32)> = {
                        ...
                        preset.values.iter().filter_map(|&(param_id, norm_val)| {
                            let ptr = match param_id {
                                "depth" => Some(params.depth.as_ptr()),
                                "rate" => Some(params.rate_hz.as_ptr()),
                                "smoothing" => Some(params.smoothing.as_ptr()),
                                "shape" => Some(params.shape.as_ptr()),
...
pub fn create_editor(params: Arc<OasisPumpParams>) -> Option<Box<dyn Editor>> {
            ...
            HStack::new(cx, |cx| {
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        ...
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Depth", Data::params, |p| &p.depth, false);
                            knob_control(cx, "Rate", Data::params, |p| &p.rate_hz, false);
                            knob_control(cx, "Smoothing", Data::params, |p| &p.smoothing, false);
                        })
                        .class("knob-row");

                        toggle_control(cx, "Shape", Data::params, |p| &p.shape);
                    })
                    .class("section");
                })
                .class("column");
