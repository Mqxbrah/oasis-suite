                    ...
                    let updates: Vec<(ParamPtr, f32)> = {
                        ...
                        preset.values.iter().filter_map(|&(param_id, norm_val)| {
                            let ptr = match param_id {
                                "drive" => Some(params.drive.as_ptr()),
                                "input_gain" => Some(params.input_gain.as_ptr()),
                                "tone" => Some(params.tone.as_ptr()),
...
pub fn create_editor(params: Arc<OasisDriveParams>) -> Option<Box<dyn Editor>> {
    ...
    create_vizia_editor(editor_state, ViziaTheming::Custom, move |cx, _| {
        ...
        VStack::new(cx, |cx| {
            ...
            HStack::new(cx, |cx| {
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "DRIVE").class("section-title");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Drive", Data::params, |p| &p.drive, false);
                            knob_control(cx, "Input", Data::params, |p| &p.input_gain, true);
                            knob_control(cx, "Tone", Data::params, |p| &p.tone, false);
                        })
                        .class("knob-row");
                    })
                    .class("section");
