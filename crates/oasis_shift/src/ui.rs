                    ...
                    let updates: Vec<(ParamPtr, f32)> = {
                        ...
                        preset.values.iter().filter_map(|&(param_id, norm_val)| {
                            let ptr = match param_id {
                                "semitones" => Some(params.semitones.as_ptr()),
                                "cents" => Some(params.cents.as_ptr()),
                                "detune" => Some(params.detune.as_ptr()),
...
pub fn create_editor(params: Arc<OasisShiftParams>) -> Option<Box<dyn Editor>> {
    ...
    create_vizia_editor(editor_state, ViziaTheming::Custom, move |cx, _| {
        ...
        VStack::new(cx, |cx| {
            ...
            HStack::new(cx, |cx| {
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "PITCH").class("section-title");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Semitones", Data::params, |p| &p.semitones, true);
                            knob_control(cx, "Cents", Data::params, |p| &p.cents, true);
                        })
                        .class("knob-row");
                ...
                VStack::new(cx, |cx| {
                    VStack::new(cx, |cx| {
                        Label::new(cx, "OPTIONS").class("section-title");
                        HStack::new(cx, |cx| {
                            knob_control(cx, "Detune", Data::params, |p| &p.detune, false);
                        })
                        .class("knob-row");
                    })
                    .class("section");
